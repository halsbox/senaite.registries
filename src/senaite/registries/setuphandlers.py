...
def post_uninstall(portal_setup):
    ...
    # 1) Hard-uncatalog (by path) from core catalogs
    hard_uncatalog_registries_structure(portal)

    # 2) Remove our types from Plone’s displayed types
    cleanup_displayed_types()

    # 3) Optional: hide the root (if still present)
...
def hard_uncatalog_registries_structure(portal):
    ...
    # catalogs to touch; add others if needed
